...
import { ArrowLeft, Calendar, Clock, Users, Phone, Mail, CheckCircle, XCircle, AlertCircle } from 'lucide-react';
...
export function RestaurantDashboard({ onBack, restaurantId, restaurantName }: RestaurantDashboardProps) {
  ...
  const loadBookings = async () => {
    ...
    try {
      const response = await fetchWithAuth(getEndpoint(`/reservations/restaurant/${restaurantId}`));
      if (!response.ok) {
        const payload = await response.json().catch(() => null);
        throw new Error(payload?.error || 'Erro ao carregar reservas');
      }
      ...
      const cancelled = data.filter((booking) => booking.status === 'cancelled').length;
      const totalGuests = data
        .filter((booking) => booking.status === 'confirmed')
        .reduce((sum, booking) => sum + booking.people_count, 0);
  ...
  const updateBookingStatus = async (bookingId: number, status: 'confirmed' | 'cancelled') => {
    try {
      const response = await fetchWithAuth(getEndpoint(`/reservations/${bookingId}/status`), {
        method: 'PUT',
        headers: {
      ...
      if (!response.ok) {
        const payload = await response.json().catch(() => null);
        throw new Error(payload?.error || 'Erro ao atualizar reserva');
      }
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-6">
      ...
      <div className="px-4 -mt-4">
        ...
        {isLoading ? (
          <div className="bg-white rounded-xl p-8 text-center border border-gray-100 text-gray-500">A carregar reservas...</div>
        ) : (
                ...
                <div key={booking.id} className="bg-white rounded-xl p-4 border border-gray-100">
                  ...
                  <div className="grid grid-cols-3 gap-3 mb-4 p-3 bg-gray-50 rounded-lg">
                    <div>
                      ...
                      <p className="text-sm font-medium">{booking.reservation_date}</p>
                    </div>
                    <div>
                      <div className="flex items-center gap-1 text-xs text-gray-500 mb-1">
                      ...
                      <p className="text-sm font-medium">{booking.reservation_time || '-'}</p>
                    </div>
                    <div>
                      <div className="flex items-center gap-1 text-xs text-gray-500 mb-1">
                      ...
                      <p className="text-sm font-medium">{booking.people_count}</p>
                    </div>
                  </div>
                  ...
                  {booking.notes && (
                    <div className="mb-4 p-3 bg-blue-50 border border-blue-100 rounded-lg">
                      ...
                      <p className="text-sm text-blue-800 whitespace-pre-line">{booking.notes}</p>
                    </div>
                  )}
